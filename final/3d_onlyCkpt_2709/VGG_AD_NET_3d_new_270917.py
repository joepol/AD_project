        ...
        if i%50 == 0:
           ...
           #print("step %d, validation accuracy, %f" %(i,valid_accuracy*100))
            ...
            for j in range(2):
                ...
                #valid_summary, valid_accuracy_cur = sess.run([merged,accuracy], feed_dict={x: Xtr_validate, y_: Ytr_validate})
                #valid_writer.add_summary(valid_summary, j+batch_num*31)		## we need to make this number unique - concatenate it using train_iter+batch_num+j
...
for train_iter in range(25000):
    ...
    for i in range(batch_num):
        ...
        if (i%25==0):
            ...
            if i%26 == 0:
                ...
                train_summary, _ = sess.run([merged, train_step], feed_dict={x:Xtr_train, y_:Ytr_train},options=run_options,run_metadata=run_metadata) #,options=run_options,run_metadata=run_metadata
    ...
    #print("iteration valid accuracy: %f" %(batch_valid_accuracy/(51)))
    batch_valid_accuracy = 0
    a = saver.save(sess, DIR+"3d_tb_270917_mode.ckpt")
    ##print("saved information is:", a)
    ##print("saved session weights to file")
