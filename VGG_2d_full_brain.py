...
#     expanded_pic = np.expand_dims(train_tmp[i],0)
#     train_all.append(expanded_pic)
#     if (i%62):
#         np.vstack(train)
...
data_set_x = []
for i in range(96):
    data_set_x.append(train_tmp[i,(i+1)*62])
...
data_set_y = np.asarray(trX_subjs_train.diagnosis)
...
# #######################################
            #Building network#
# #######################################
x = tf.placeholder(tf.float32, shape=[None, 62,96 ,96])
y_ = tf.placeholder(tf.float32, shape = [None, 3])
x_image=x

#
# ##### first convolutional later #####
# W_conv1 = weight_variable([15,15, 62, 32], "W_conv1")
...
# W_conv2 = weight_variable([15, 15, 62*32, 64], "W_conv2")
...
# W_fc1 = weight_variable([6 * 6 * 128, 64],"W_fc1")
# b_fc1 = bias_variable([64],"b_fc1")
#
# h_pool4_flat = tf.reshape(h_pool4, [-1, 6 * 6 * 128])
...
conv1 = tf.layers.conv2d(x, filters=32, kernel_size=(3,3), strides=(2,2), activation='relu', data_format='channe')
conv2 = tf.layers.conv2d(conv1, filters=32, kernel_size=(3,3), strides=(2,2), activation='relu', data_format='channe')
conv3 = tf.layers.conv2d(conv2, filters=32, kernel_size=(3,3), strides=(2,2), activation='relu', data_format='channe')
conv4 = tf.layers.conv2d(conv3, filters=32, kernel_size=(3,3), strides=(2,2), activation='relu', data_format='channe')
flat = tf.cotrib.layers.flatten(conv1)
fc1 = tf.layers.dense(flat, 256, activation='relu')
fc2 = tf.layers.dense(fc1, 256, activation='relu')
fc3 = tf.layers.dense(fc2, y_.shape[1], activation='relu')
...
cross_entropy = tf.reduce_mean(
      tf.nn.softmax_cross_entropy_with_logits(labels=y_, logits=fc3))
...
#     for i in range(40): # duplicating diagnosis for each slice
...
#     if validY_trim_after_dup[i] != 2:
#         label = [0,0]
#         if (validY_trim_after_dup[i]==1):
#             label[0] = 1
#         else:
#             label[1] = 1
#         label = np.asarray(label)
