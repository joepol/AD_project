...
def bias_variable(shape, bias_name):
    ...
    return tf.get_variable(bias_name, shape, tf.float32, tf.constant_initializer(0.1, dtype=tf.float32))
...
# def conv3d(x, W, layer_name):
#     return tf.nn.conv3d((x,layer_name), W, [1, 1, 1, 1, 1], padding='SAME')
#
# def max_pool_3x3(x,layer_name):
#     return tf.nn.max_pool3d((x,layer_name), ksize=[1, 3, 3, 3, 1], strides=[1, 2, 2, 2, 1], padding='SAME')
...
# trY_all = []
# for n in range(96):		# len(trY) = 2109
...
# #######################################
            #Building network#
# #######################################
x = tf.placeholder(tf.float32, shape=[None, 62, 96 , 96 , 1])
y_ = tf.placeholder(tf.float32, shape = [None, 2])		
x_image=x


##### first convolutional later #####
W_conv1 = weight_variable([15,15,15 , 1, 32], "W_conv1")
...
h_pool1 = max_pool_3x3(h_conv1,"first layer")
# input: 96X96X32 -> max_pool -> 48X48X32

##### second convolutional layer #####
W_conv2 = weight_variable([15, 15,15, 32, 64], "W_conv2")
...
h_pool2 = max_pool_3x3(h_conv2,"second layer")
...
h_pool3 = max_pool_3x3(h_conv3,"third layer")
...
h_pool4 = max_pool_3x3(h_conv4,"fourth layer")
# input is 12X12X128 ----> 6X6X128


# first fully connected layer
W_fc1 = weight_variable([6 * 6 *2* 128, 64],"W_fc1")
b_fc1 = bias_variable([64],"b_fc1")

h_pool4_flat = tf.reshape(h_pool4, [-1, 6 * 6 *2* 128])
